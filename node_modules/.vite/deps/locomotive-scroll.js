//#region node_modules/locomotive-scroll/dist/locomotive-scroll.esm.js
...
var _default = /* @__PURE__ */ function() {
	function _default() {
		...
		if (this.isMobile) if (this.isTablet) this.context = "tablet";
	...
	_createClass(_default, [
		...
		},
		{
			...
			value: function checkContext() {
				...
				if (oldContext != this.context) {
					if ((oldContext == "desktop" ? this.smooth : this[oldContext].smooth) != (this.context == "desktop" ? this.smooth : this[this.context].smooth)) window.location.reload();
				}
			}
		},
		{
		...
		},
		{
			...
			value: function setScrollTo(event) {
				...
				this.scrollTo(event.currentTarget.getAttribute("data-".concat(this.name, "-href")) || event.currentTarget.getAttribute("href"), { offset: event.currentTarget.getAttribute("data-".concat(this.name, "-offset")) });
			}
		},
		{
			key: "addElements",
		...
		},
		{
			...
			value: function checkEvent(event) {
				...
				list.forEach(function(func) {
					switch (name) {
						case "scroll": return func(_this5.instance);
						case "call": return func(_this5.callValue, _this5.callWay, _this5.callObj);
						default: return func();
					}
				});
			}
	...
	_createClass(_default, [
		...
		},
		{
			...
			value: function addElements() {
				var _this3 = this;
				this.els = {};
				this.el.querySelectorAll("[data-" + this.name + "]").forEach(function(el, index) {
					...
					var targetEl;
					if (target !== void 0) targetEl = document.querySelector("".concat(target));
					else targetEl = el;
					...
					else if (repeat != void 0) repeat = true;
		...
		},
		{
			...
			value: function getRelativeOffset(offset) {
				...
				if (offset) for (var i = 0; i < offset.length; i++) if (typeof offset[i] == "string") if (offset[i].includes("%")) relativeOffset[i] = parseInt(offset[i].replace("%", "") * this.windowHeight / 100);
		...
		},
		{
			...
			value: function scrollTo(target) {
				...
				if (typeof target === "string") if (target === "top") target = this.html;
				else if (target === "bottom") target = this.html.offsetHeight - window.innerHeight;
				else {
					target = document.querySelector(target);
					if (!target) return;
				}
				else if (typeof target === "number") target = parseInt(target);
				...
				if (callback) if (isTargetReached()) {
					callback();
					return;
				} else window.addEventListener("scroll", function onScroll() {
					if (isTargetReached()) {
	...
	(function() {
		...
		root.Lethargy = (function() {
			...
			Lethargy.prototype.isInertia = function(direction) {
				var lastDeltas = direction === -1 ? this.lastDownDeltas : this.lastUpDeltas, lastDeltasNew, lastDeltasOld, newAverage, newSum, oldAverage, oldSum;
...
var bindallStandalone = function(object) {
	...
	if (functions.length === 0) {
		for (var method in object) if (hasOwnProperty$1.call(object, method)) {
...
function VirtualScroll(options) {
	...
	if (this.options.passive !== void 0) this.listenerOptions = { passive: this.options.passive };
}
	...
	_createClass(_default, [
				...
				switch (e.keyCode) {
					case keyCodes$1.TAB:
						requestAnimationFrame(function() {
							...
							_this3.html.scrollLeft = 0;
							document.body.scrollLeft = 0;
							_this3.scrollTo(document.activeElement, { offset: -window.innerHeight / 2 });
						});
						break;
					case keyCodes$1.UP:
					...
					case keyCodes$1.SPACE:
						if (this.isActiveElementScrollSensitive()) if (e.shiftKey) this.instance.delta[this.directionAxis] -= window.innerHeight;
		...
		},
		{
			...
			value: function updateScroll(e) {
				...
				else if (this.instance.scroll[this.directionAxis] > this.instance.limit[this.directionAxis]) this.setScroll(this.instance.scroll[this.directionAxis], this.instance.limit[this.directionAxis]);
		...
		},
		{
			...
			value: function addElements() {
				...
				this.el.querySelectorAll("[data-".concat(this.name, "]")).forEach(function(el, index) {
					...
					if (offset) if (_this6.direction === "horizontal") {
						for (var i = 0; i < offset.length; i++) if (typeof offset[i] == "string") if (offset[i].includes("%")) relativeOffset[i] = parseInt(offset[i].replace("%", "") * _this6.windowWidth / 100);
						else relativeOffset[i] = parseInt(offset[i]);
						else relativeOffset[i] = offset[i];
						left = left + relativeOffset[0];
						right = right - relativeOffset[1];
					} else {
						for (var i = 0; i < offset.length; i++) if (typeof offset[i] == "string") if (offset[i].includes("%")) relativeOffset[i] = parseInt(offset[i].replace("%", "") * _this6.windowHeight / 100);
						else relativeOffset[i] = parseInt(offset[i]);
						else relativeOffset[i] = offset[i];
						top = top + relativeOffset[0];
						bottom = bottom - relativeOffset[1];
		...
		},
		{
			...
			value: function transformElements(isForced) {
				...
				Object.entries(this.parallaxElements).forEach(function(_ref5) {
					...
					if (current.sticky) if (current.inView) if (_this8.direction === "horizontal") transformDistance = _this8.instance.scroll.x - current.left + window.innerWidth;
					else transformDistance = _this8.instance.scroll.y - current.top + window.innerHeight;
					else if (_this8.direction === "horizontal") if (_this8.instance.scroll.x < current.left - window.innerWidth && _this8.instance.scroll.x < current.left - window.innerWidth / 2) transformDistance = 0;
					...
					else if (_this8.instance.scroll.y < current.top - window.innerHeight && _this8.instance.scroll.y < current.top - window.innerHeight / 2) transformDistance = 0;
					...
					if (transformDistance !== false) if (current.direction === "horizontal" || _this8.direction === "horizontal" && current.direction !== "vertical") _this8.transform(current.el, transformDistance, 0, isForced ? false : current.delay);
		...
		},
		{
			...
			value: function scrollTo(target) {
				...
				if (typeof target === "string") if (target === "top") target = 0;
				...
				if (typeof target !== "number") {
					if (!getParents(target).includes(this.el)) return;
					var targetBCR = target.getBoundingClientRect();
					var offsetTop = targetBCR.top;
					var offsetLeft = targetBCR.left;
					var parentSection = getParents(target).find(function(candidate) {
					...
					if (parentSection) parentSectionOffset = getTranslate(parentSection)[this.directionAxis];
					else parentSectionOffset = -this.instance.scroll[this.directionAxis];
				...
				var scrollDiff = Math.max(0, Math.min(offset, this.instance.limit[this.directionAxis])) - scrollStart;
				var render = function render(p) {
					if (disableLerp) if (_this9.direction === "horizontal") _this9.setScroll(scrollStart + scrollDiff * p, _this9.instance.delta.y);
...
//#endregion
export { Native, Smooth, Smooth as default };
